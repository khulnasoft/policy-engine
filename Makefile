...
define LDFLAGS
	-X \"github.com/khulnasoft/policy-engine/pkg/version.Version=$(CURRENT_VERSION)-dev\"
endef
...
release:
	...
	@echo "Go to https://github.com/khulnasoft/policy-engine/compare/release/$(PLAIN_VERSION)?expand=1"
...
vendor_terraform:
	...
	find pkg/internal/terraform/ -name '*.go' \
		-exec sed -i".bak" 's#github\.com/hashicorp/terraform/internal/#github.com/khulnasoft/policy-engine/pkg/internal/terraform/#' '{}' \;
	find pkg/internal/terraform/ -name '*.go' \
		-exec sed -i".bak" 's#github\.com/hashicorp/terraform/version#github.com/khulnasoft/policy-engine/pkg/internal/terraform/version#' '{}' \;
